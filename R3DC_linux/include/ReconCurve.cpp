...
void ReconCurve::CalculateCDE3(vector<int> combineStartIdx, vector<ReconCurve> subcurves, vector<ProjectImage> Views, vector<int> partCurveNum, string outpath, string model) {
	...
	struct stat status;
	int stat_result1 = stat(outpath.c_str(), &status);

	// if (stat_result1 == 0 && !((status.st_mode & S_IFMT) == S_IFDIR)) {
	if (stat_result1 != 0) {
		mkdir(outpath.c_str(), 0766);
	}
	else {
		// cout << outpath << endl;
		// cout << stat_result1 << endl;
		// cout << status.st_mode;
		cout << "exists" << endl;
	}
	...
	struct stat status2;
	int stat_result2 = stat(outpath.c_str(), &status2);

	// if (stat_result2 == 0 && !((status.st_mode & S_IFMT) & S_IFDIR)) {
	if (stat_result2 != 0) {	
		mkdir(outpath.c_str(), 0766);
	}
	else {
		// cout << outpath << endl;
		// cout << stat_result2 << endl;
		// cout << status2.st_mode << endl << S_IFMT << endl << S_IFDIR;
		cout << "exists2" << endl;
	}
	...
	for (int i = 0; i < curveNum; i++) {
		...
		C[i] = sqrt(((float)sumdist[i]) / subcurves[i].curve.size());
		viewDists[i] = tempDists;
		for (int k = 0; k < viewNum; k++) {
			viewDists[i][k] = sqrt(viewDists[i][k] / subcurves[i].curve.size());
		}
...
void ReconCurve::DeleteFileInDire(string path) {
	string strPath = path + "/notiso_color";
	...
	if (status.st_mode & S_IFDIR) {
		string snotiso2 = "rm " + strPath + "/*.off -f > log.txt";
		system(snotiso2.c_str());
	}
	else {
		mkdir(strPath.c_str(), 0766);
	}
}
